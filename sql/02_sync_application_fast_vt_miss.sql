...
SELECT
    e.application_no, e.mobile_token, 'ng01', CAST(e.app_id_num AS INT), '',
    e.user_id, e.group_user_id, e.sn,
    CAST(0 AS TINYINT),
    CAST(CASE WHEN e.re_loan = 0 THEN 1 ELSE 0 END AS TINYINT),
    CAST(0 AS TINYINT),
...
FROM (
    SELECT
        ...
        s.app_id_num, s.device_uuid, s.session_id, s.re_loan, s.product_id,
        s.period_days, s.period_count, s.order_time, s.disburse_time, s.settled_time,
        s.last_repayment_time, s.credit_limit_minor, s.loan_amount_minor, s.principal_minor,

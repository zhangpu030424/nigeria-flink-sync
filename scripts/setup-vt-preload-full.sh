...
echo ">> [2/5] 仅建 vt_token_cache 表（stream 模式由脚本从源表直查，无需 init_all 预灌）"
run_sql_file sql/ddl/vt_token_cache.sql

echo ">> [3/5] stream 模式批量 /v2t（默认 20 workers × 5万/请求）"
./scripts/vt-preload.sh --mode stream --vt-type all --skip-count \
  --workers "${VT_PRELOAD_WORKERS:-20}" \
  --batch-size "${VT_PRELOAD_BATCH_SIZE:-1000000}" \

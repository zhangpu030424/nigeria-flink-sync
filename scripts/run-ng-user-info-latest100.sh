...
# 用法: bash scripts/run-ng-user-info-latest100.sh
#       LM_PICK_N=100 bash scripts/run-ng-user-info-latest100.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
bash scripts/refresh-lm-user-info-latest100.sh
...
echo "[$(date '+%F %T')] 提交 Flink Job: sql/04_sync_ng_user_info_latest100.sql"

...
# user / bankcard / product / application / loan / user_info 增量 CDC server-id
# 每个 source 的 server-id 个数必须 ≥ 并行度；相邻段间隔 20，支持并行度 ≤20
...
export CDC_SERVER_ID_USER_ADJUST="${CDC_SERVER_ID_USER_ADJUST:-5521-$((5520 + _sid))}"
export CDC_SERVER_ID_BANKCARD_INFO="${CDC_SERVER_ID_BANKCARD_INFO:-5541-$((5540 + _sid))}"
export CDC_SERVER_ID_BANKCARD_VT="${CDC_SERVER_ID_BANKCARD_VT:-5561-$((5560 + _sid))}"
export CDC_SERVER_ID_USER_PRODUCT="${CDC_SERVER_ID_USER_PRODUCT:-5581-$((5580 + _sid))}"
export CDC_SERVER_ID_APP_ORDER="${CDC_SERVER_ID_APP_ORDER:-5601-$((5600 + _sid))}"
export CDC_SERVER_ID_APP_USER="${CDC_SERVER_ID_APP_USER:-5621-$((5620 + _sid))}"
export CDC_SERVER_ID_APP_BANK="${CDC_SERVER_ID_APP_BANK:-5641-$((5640 + _sid))}"
export CDC_SERVER_ID_APP_PERSONAL="${CDC_SERVER_ID_APP_PERSONAL:-5661-$((5660 + _sid))}"
export CDC_SERVER_ID_APP_DEVICE="${CDC_SERVER_ID_APP_DEVICE:-5681-$((5680 + _sid))}"
export CDC_SERVER_ID_APP_REPAY="${CDC_SERVER_ID_APP_REPAY:-5701-$((5700 + _sid))}"
export CDC_SERVER_ID_APP_RISK="${CDC_SERVER_ID_APP_RISK:-5721-$((5720 + _sid))}"
export CDC_SERVER_ID_APP_INSTALLMENT="${CDC_SERVER_ID_APP_INSTALLMENT:-5741-$((5740 + _sid))}"
export CDC_SERVER_ID_LOAN_INSTALLMENT="${CDC_SERVER_ID_LOAN_INSTALLMENT:-5801-$((5800 + _sid))}"
export CDC_SERVER_ID_LOAN_ORDER="${CDC_SERVER_ID_LOAN_ORDER:-5821-$((5820 + _sid))}"
export CDC_SERVER_ID_LOAN_REPAY="${CDC_SERVER_ID_LOAN_REPAY:-5841-$((5840 + _sid))}"
# id_mapping 多源 CDC（旧 CDC_SERVER_ID_ID_MAPPING 兼容映射到 USER）
export CDC_SERVER_ID_IDMAP_USER="${CDC_SERVER_ID_IDMAP_USER:-${CDC_SERVER_ID_ID_MAPPING:-5861-$((5860 + _sid))}}"
export CDC_SERVER_ID_IDMAP_ORDER="${CDC_SERVER_ID_IDMAP_ORDER:-5881-$((5880 + _sid))}"
export CDC_SERVER_ID_IDMAP_BANK="${CDC_SERVER_ID_IDMAP_BANK:-5901-$((5900 + _sid))}"
export CDC_SERVER_ID_IDMAP_PERSONAL="${CDC_SERVER_ID_IDMAP_PERSONAL:-5921-$((5920 + _sid))}"
export CDC_SERVER_ID_IDMAP_DEVICE="${CDC_SERVER_ID_IDMAP_DEVICE:-5941-$((5940 + _sid))}"
export CDC_SERVER_ID_ID_MAPPING="${CDC_SERVER_ID_IDMAP_USER}"

export LM_LOAN_SYNC_SHARD_MAX="${LM_LOAN_SYNC_SHARD_MAX:-$((FLINK_PARALLELISM - 1))}"

# 目标库 JDBC：抗短暂断连（Communications link failure）
# autoReconnect + tcpKeepAlive；socketTimeout=0 避免长 batch 被误杀
export TARGET_JDBC_PARAMS="${TARGET_JDBC_PARAMS:-useSSL=false&allowPublicKeyRetrieval=true&rewriteBatchedStatements=true&autoReconnect=true&maxReconnects=10&failOverReadOnly=false&connectTimeout=30000&socketTimeout=0&tcpKeepAlive=true}"
export FLINK_SINK_MAX_RETRIES="${FLINK_SINK_MAX_RETRIES:-15}"
export FLINK_JDBC_RETRY_TIMEOUT="${FLINK_JDBC_RETRY_TIMEOUT:-120s}"
# 维表 Lookup 缓存：对齐秒级 SLA（过短会打爆源库；过长则 CDC 触发前可能读到旧维）
export LOOKUP_CACHE_TTL="${LOOKUP_CACHE_TTL:-30s}"

VARS='${SOURCE_MYSQL_HOST} ${SOURCE_MYSQL_PORT} ${SOURCE_MYSQL_USER} ${SOURCE_MYSQL_PASSWORD} ${SOURCE_MYSQL_DATABASE} ${LM_MYSQL_HOST} ${LM_MYSQL_PORT} ${LM_MYSQL_USER} ${LM_MYSQL_PASSWORD} ${LM_MYSQL_DATABASE} ${LM_LOAN_SYNC_SHARD_MAX} ${LM_CORE_MYSQL_HOST} ${LM_CORE_MYSQL_PORT} ${LM_CORE_MYSQL_USER} ${LM_CORE_MYSQL_PASSWORD} ${LM_CORE_MYSQL_DATABASE} ${LM_MIGRATION_LIMIT} ${LM_MIGRATION_LIMIT_CLAUSE} ${LM_MIGRATION_ID_UPPER} ${LM_USER_ID_RANGE_CLAUSE} ${LM_SRC_TABLE_READY} ${LM_USER_REG_IP_TABLE} ${LM_CDC_SERVER_ID_USER} ${LM_CDC_SERVER_ID_USER_DATA} ${LM_CDC_SERVER_ID_LUP} ${LM_CDC_SERVER_ID_DAC} ${LM_CDC_SERVER_ID_URI} ${CDC_SERVER_ID_UI_DIRTY} ${CDC_SERVER_ID_UI_USER} ${CDC_SERVER_ID_UI_PERSONAL} ${CDC_SERVER_ID_UI_WORK} ${CDC_SERVER_ID_UI_EMERGENCY} ${CDC_SERVER_ID_UI_CREDIT} ${CDC_SERVER_ID_UI_VT} ${CDC_SERVER_ID_UI_ADJUST} ${CDC_SERVER_ID_USER_MAIN} ${CDC_SERVER_ID_USER_ADJUST} ${CDC_SERVER_ID_BANKCARD_INFO} ${CDC_SERVER_ID_BANKCARD_VT} ${CDC_SERVER_ID_USER_PRODUCT} ${CDC_SERVER_ID_APP_ORDER} ${CDC_SERVER_ID_APP_USER} ${CDC_SERVER_ID_APP_BANK} ${CDC_SERVER_ID_APP_PERSONAL} ${CDC_SERVER_ID_APP_DEVICE} ${CDC_SERVER_ID_APP_REPAY} ${CDC_SERVER_ID_APP_RISK} ${CDC_SERVER_ID_APP_INSTALLMENT} ${CDC_SERVER_ID_LOAN_INSTALLMENT} ${CDC_SERVER_ID_LOAN_ORDER} ${CDC_SERVER_ID_LOAN_REPAY} ${CDC_SERVER_ID_ID_MAPPING} ${CDC_SERVER_ID_IDMAP_USER} ${CDC_SERVER_ID_IDMAP_ORDER} ${CDC_SERVER_ID_IDMAP_BANK} ${CDC_SERVER_ID_IDMAP_PERSONAL} ${CDC_SERVER_ID_IDMAP_DEVICE} ${TARGET_MYSQL_HOST} ${TARGET_MYSQL_PORT} ${TARGET_MYSQL_USER} ${TARGET_MYSQL_PASSWORD} ${TARGET_MYSQL_DATABASE} ${TARGET_JDBC_PARAMS} ${FLINK_PARALLELISM} ${FLINK_MINI_BATCH_SIZE} ${FLINK_SINK_BUFFER_ROWS} ${FLINK_SINK_MAX_RETRIES} ${FLINK_JDBC_RETRY_TIMEOUT} ${FLINK_CDC_CHUNK_SIZE} ${FLINK_CDC_FETCH_SIZE} ${FLINK_CHECKPOINT_INTERVAL} ${FLINK_CHECKPOINT_TIMEOUT} ${CDC_STARTUP_MODE} ${CDC_STARTUP_TIMESTAMP_MILLIS} ${USER_INFO_DIRTY_COALESCE_SEC} ${LOOKUP_CACHE_TTL}'

...
# user_info 增量多 CDC 分片（5400+，每片单 server-id）
_ui_base="${CDC_SERVER_ID_UI_BASE:-5400}"
_ui_span="${CDC_SERVER_ID_UI_SPAN:-4}"
_ui_dirty_step="${CDC_SERVER_ID_UI_DIRTY_STEP:-10}"
# 脏队列表 CDC 关闭 incremental snapshot 时 server-id 须为单值（非范围，否则 NumberFormatException）
_cdc_dirty_single() {
  local v="$1"
  if [[ "$v" == *-* ]]; then
    echo "${v%%-*}"
...
export CDC_SERVER_ID_UI_DIRTY_0="$(_cdc_dirty_single "${CDC_SERVER_ID_UI_DIRTY_0:-${CDC_SERVER_ID_UI_DIRTY:-$((_ui_base + 1))}}")"
export CDC_SERVER_ID_UI_DIRTY_1="$(_cdc_dirty_single "${CDC_SERVER_ID_UI_DIRTY_1:-$((_ui_base + 1 + _ui_dirty_step))}")"
export CDC_SERVER_ID_UI_DIRTY_2="$(_cdc_dirty_single "${CDC_SERVER_ID_UI_DIRTY_2:-$((_ui_base + 1 + _ui_dirty_step * 2))}")"
export CDC_SERVER_ID_UI_DIRTY_3="$(_cdc_dirty_single "${CDC_SERVER_ID_UI_DIRTY_3:-$((_ui_base + 1 + _ui_dirty_step * 3))}")"
export CDC_SERVER_ID_UI_DIRTY="${CDC_SERVER_ID_UI_DIRTY_0}"

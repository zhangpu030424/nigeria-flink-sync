...
流程：
  ...
  4. 加 --apply 才写入目标库（需 UPDATE 权限；ng-export 只读时用 --export-sql）
...
Usage:
  ...
  python3 scripts/backfill_user_info_id_number_from_bvn.py \\
    --env ./.env \\
    --input /path/无标题.xls \\
    --export-sql /tmp/update_id_number.sql

  # 有写权限账号（.env 配 TARGET_WRITE_MYSQL_USER/PASSWORD）或 DBA 执行 SQL
  mysql -h ... -u admin -p ng < /tmp/update_id_number.sql
"""
...
def update_sql(p: PlanRow) -> str:
    return (
        "UPDATE `user_info` SET `id_number`={token} WHERE `user_id`={uid};".format(
            token=sql_literal(p.token),
            uid=int(p.user_id),
        )
    )


def write_db_cfg(cfg: Dict[str, Any]) -> Dict[str, Any]:
    """若 .env 配置了 TARGET_WRITE_MYSQL_* 则用于 UPDATE。"""
    out = dict(cfg)
    user = cfg.get("TARGET_WRITE_MYSQL_USER") or cfg.get("TARGET_MYSQL_WRITE_USER")
    pwd = cfg.get("TARGET_WRITE_MYSQL_PASSWORD") or cfg.get("TARGET_MYSQL_WRITE_PASSWORD")
    if user:
        out["TARGET_MYSQL_USER"] = user
    if pwd:
        out["TARGET_MYSQL_PASSWORD"] = pwd
    return out


def connect_target_write(cfg: Dict[str, Any]):
    wcfg = write_db_cfg(cfg)
    return pymysql.connect(
        host=wcfg["TARGET_MYSQL_HOST"],
        port=int(wcfg.get("TARGET_MYSQL_PORT") or 3306),
        user=wcfg["TARGET_MYSQL_USER"],
        password=wcfg["TARGET_MYSQL_PASSWORD"],
        database=wcfg["TARGET_MYSQL_DATABASE"],
        charset="utf8mb4",
...
def export_update_sql(plans: Sequence[PlanRow], path: Path, *, database: str) -> int:
    todo = [p for p in plans if p.action == "update"]
    lines = [
        "-- generated by backfill_user_info_id_number_from_bvn.py",
        "USE `{0}`;".format(database.replace("`", "")),
        "START TRANSACTION;",
    ]
    lines.extend(update_sql(p) for p in todo)
    lines.append("COMMIT;")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    log("exported {0} UPDATE statements -> {1}".format(len(todo), path))
    return len(todo)


def apply_via_mysql(cfg: Dict[str, Any], plans: Sequence[PlanRow]) -> int:
    todo = [p for p in plans if p.action == "update"]
    if not todo:
        return 0
    wcfg = write_db_cfg(cfg)
    sql = "\n".join(update_sql(p) for p in todo) + "\n"
    cmd = [
        "mysql",
        "-h", str(wcfg["TARGET_MYSQL_HOST"]),
        "-P", str(int(wcfg.get("TARGET_MYSQL_PORT") or 3306)),
        "-u", str(wcfg["TARGET_MYSQL_USER"]),
        str(wcfg["TARGET_MYSQL_DATABASE"]),
        "--connect-timeout=60",
        "--init-command", _MYSQL_INIT,
    ]
    env = os.environ.copy()
    env["MYSQL_PWD"] = str(wcfg["TARGET_MYSQL_PASSWORD"])
    proc = subprocess.run(cmd, input=sql, env=env, capture_output=True, text=True)
    if proc.returncode != 0:
        err = (proc.stderr or proc.stdout or "mysql failed").strip()
        raise RuntimeError(err)
    log("via-mysql: executed {0} UPDATE statement(s)".format(len(todo)))
    return len(todo)
...
def main() -> None:
    ...
    parser.add_argument(
        "--export-sql",
        metavar="FILE",
        help="导出 UPDATE SQL（ng-export 无 UPDATE 权限时用此方式）",
    )
    parser.add_argument(
        "--via-mysql",
        action="store_true",
        help="用 mysql 客户端执行 UPDATE（配合 TARGET_WRITE_MYSQL_*）",
    )
    args = parser.parse_args()

    if args.apply and args.export_sql:
        raise SystemExit("use either --apply or --export-sql, not both")
    ...
    try:
        ...
        todo = len([p for p in plans if p.action == "update"])
        updated = 0

        if args.export_sql:
            export_update_sql(
                plans,
                Path(args.export_sql),
                database=str(cfg["TARGET_MYSQL_DATABASE"]),
            )
            log(
                "run with a user that has UPDATE on user_info, e.g.:\n"
                "  mysql -h {0} -P {1} -u <write_user> -p {2} < {3}".format(
                    cfg["TARGET_MYSQL_HOST"],
                    int(cfg.get("TARGET_MYSQL_PORT") or 3306),
                    cfg["TARGET_MYSQL_DATABASE"],
                    args.export_sql,
                )
            )
        elif args.apply:
            write_cfg = write_db_cfg(cfg)
            using_write_user = (
                write_cfg.get("TARGET_MYSQL_USER") != cfg.get("TARGET_MYSQL_USER")
            )
            if using_write_user:
                log("UPDATE via TARGET_WRITE_MYSQL_USER={0}".format(write_cfg["TARGET_MYSQL_USER"]))
            else:
                log(
                    "WARN: no TARGET_WRITE_MYSQL_USER; UPDATE as {0}".format(
                        cfg["TARGET_MYSQL_USER"]
                    )
                )
            env_util.close_conn(target)
            target = None
            write_conn = connect_target_write(cfg)
            try:
                if args.via_mysql:
                    updated = apply_via_mysql(cfg, plans)
                else:
                ...
                if e.args and e.args[0] == 1142:
                    raise SystemExit(
                        "UPDATE denied for {0}; use --export-sql or set "
                        "TARGET_WRITE_MYSQL_USER/PASSWORD in .env".format(
                            write_cfg["TARGET_MYSQL_USER"]
                        )
                    ) from e
                raise
            finally:
                env_util.close_conn(write_conn)
            log("done: attempted={0} rowcount={1}".format(todo, updated))
        else:
            log(
                "dry-run: VT tokens are real; add --apply or --export-sql for {0} rows".format(
                    todo
                )
            )
    finally:

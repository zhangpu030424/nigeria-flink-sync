...
CACHE_RECONCILE_SQL = """
SELECT id, vt_type, HEX(raw_value) AS raw_hex, IFNULL(token, ''), IFNULL(masking, '')
FROM vt_token_cache
WHERE status = {ok}
  AND id > {last_id}
...
def parse_cache_rows(rows: List[str]) -> Tuple[List[Dict[str, Any]], int]:
...
def reconcile_type(
    ...
    *,
    ...
    total_hint = -1
    if not skip_count:
        total_hint = int(mysql_query(host, port, user, password, database, _count_sql(vt_code, include_empty_token))[0])
    ...
    log(f"{vt_type_name} 流式对账：每页 {round_batch} 条，拉完即调 /v2t")
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        while True:
            ...
            page_no += 1
            last_id = rows[-1]["id"]
            log(f"{vt_type_name} 第 {page_no} 页 {len(rows)} 条，耗时 {fetch_sec:.1f}s，开始对账 ...")
...
def main() -> int:
    ...
    parser.add_argument("--batch-size", type=int, default=None, help="MySQL 分页拉取条数")
    ...
    parser.add_argument(
        ...
        help="各 vt_type 串行（默认 all 时多 type 并行）",
    )
    ...
    log(
        ...
        f"workers={workers} | http_batch={http_batch} | url={base_url} | 明细={log_file}",
    )
    log("只读比对，不会写入 vt_token_cache")

    run_start = time.time()
    grand = ReconcileStats()
    parallel_types = len(types) > 1 and not args.sequential_types
